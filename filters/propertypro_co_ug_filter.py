...
def propertypro_co_ug_filter():
    ...
    df9['Country GDP Growth Rate'] = getCalcValue('gdpGrowthRate', df9)
    df9['Country Interest Rates'] = getCalcValue('interestRates', df9)
    df9['Country Mortgage Rates'] = getCalcValue('mortgageRates', df9)
    df9['Country gdp'] = getCalcValue('countryGdp', df9)
    df9['Country gdp per capita'] = getCalcValue('countryGdpPerCapita', df9)
    df9['Fed Funds Rate'] = getCalcValue('fedFundsRate', df9)
    df9['Inflation Rate'] = getCalcValue('inflationRate', df9)
    df9['US Mortgage Rates'] = getCalcValue('usMortgageRates', df9)
    df9['Mortgage rate for 10 years'] = getCalcValue('mortgageRateFor10Years', df9)
    df9['Mortgage rate for 15 years'] = getCalcValue('mortgageRateFor15Years', df9)
    df9['Mortgage rate for 20 years'] = getCalcValue('mortgageRateFor20Years', df9)
    df9['Mortgage rate for 30 years'] = getCalcValue('mortgageRateFor30Years', df9)
    df9['Interbank Rate'] = getCalcValue('interbankRate', df9)
    df9['Country Remittance'] = getCalcValue('countryRemittance', df9)
    df9['Tourist Arrivals'] = getCalcValue('touristArrivals', df9)
    df9['Tourist Dollars (in thousands)'] = getCalcValue('touristDollars', df9)
    df9['Foreign Direct Investment'] = getCalcValue('foreignDirectInvestment', df9)
    df9['Personal Income Tax Rate'] = getCalcValue('personalIncomeTaxRate', df9)
    df9['Construction GDP'] = getCalcValue('constructionGDP', df9)
    df9['Unemployment Rate'] = getCalcValue('unemploymentRate', df9)
    df9['Minimum Wage (USD/month)'] = getCalcValue('minimumWage(USD/month)', df9)
    df9['Government Debt To GDP'] = getCalcValue('governmentDebtToGDP', df9)
    df9['Building Permits'] = getCalcValue('buildingPermits', df9)
    df9['Price To Rent Ratio'] = getCalcValue('priceToRentRatio', df9)
