...
def propertypro_co_ke_filter():
    ...
    df8['Country GDP Growth Rate'] = getCalcValue('gdpGrowthRate', df8)
    df8['Country Interest Rates'] = getCalcValue('interestRates', df8)
    df8['Country Mortgage Rates'] = getCalcValue('mortgageRates', df8)
    df8['Country gdp'] = getCalcValue('countryGdp', df8)
    df8['Country gdp per capita'] = getCalcValue('countryGdpPerCapita', df8)
    df8['Fed Funds Rate'] = getCalcValue('fedFundsRate', df8)
    df8['Inflation Rate'] = getCalcValue('inflationRate', df8)
    df8['US Mortgage Rates'] = getCalcValue('usMortgageRates', df8)
    df8['Mortgage rate for 10 years'] = getCalcValue('mortgageRateFor10Years', df8)
    df8['Mortgage rate for 15 years'] = getCalcValue('mortgageRateFor15Years', df8)
    df8['Mortgage rate for 20 years'] = getCalcValue('mortgageRateFor20Years', df8)
    df8['Mortgage rate for 30 years'] = getCalcValue('mortgageRateFor30Years', df8)
    df8['Interbank Rate'] = getCalcValue('interbankRate', df8)
    df8['Country Remittance'] = getCalcValue('countryRemittance', df8)
    df8['Tourist Arrivals'] = getCalcValue('touristArrivals', df8)
    df8['Tourist Dollars (in thousands)'] = getCalcValue('touristDollars', df8)
    df8['Foreign Direct Investment'] = getCalcValue('foreignDirectInvestment', df8)
    df8['Personal Income Tax Rate'] = getCalcValue('personalIncomeTaxRate', df8)
    df8['Construction GDP'] = getCalcValue('constructionGDP', df8)
    df8['Unemployment Rate'] = getCalcValue('unemploymentRate', df8)
    df8['Minimum Wage (USD/month)'] = getCalcValue('minimumWage(USD/month)', df8)
    df8['Government Debt To GDP'] = getCalcValue('governmentDebtToGDP', df8)
    df8['Building Permits'] = getCalcValue('buildingPermits', df8)
    df8['Price To Rent Ratio'] = getCalcValue('priceToRentRatio', df8)
