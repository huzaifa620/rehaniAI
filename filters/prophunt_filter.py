...
def prophunt_filter():
    ...
    df7['Country GDP Growth Rate'] = getCalcValue('gdpGrowthRate', df7)
    df7['Country Interest Rates'] = getCalcValue('interestRates', df7)
    df7['Country Mortgage Rates'] = getCalcValue('mortgageRates', df7)
    df7['Country gdp'] = getCalcValue('countryGdp', df7)
    df7['Country gdp per capita'] = getCalcValue('countryGdpPerCapita', df7)
    df7['Fed Funds Rate'] = getCalcValue('fedFundsRate', df7)
    df7['Inflation Rate'] = getCalcValue('inflationRate', df7)
    df7['US Mortgage Rates'] = getCalcValue('usMortgageRates', df7)
    df7['Mortgage rate for 10 years'] = getCalcValue('mortgageRateFor10Years', df7)
    df7['Mortgage rate for 15 years'] = getCalcValue('mortgageRateFor15Years', df7)
    df7['Mortgage rate for 20 years'] = getCalcValue('mortgageRateFor20Years', df7)
    df7['Mortgage rate for 30 years'] = getCalcValue('mortgageRateFor30Years', df7)
    df7['Interbank Rate'] = getCalcValue('interbankRate', df7)
    df7['Country Remittance'] = getCalcValue('countryRemittance', df7)
    df7['Tourist Arrivals'] = getCalcValue('touristArrivals', df7)
    df7['Tourist Dollars (in thousands)'] = getCalcValue('touristDollars', df7)
    df7['Foreign Direct Investment'] = getCalcValue('foreignDirectInvestment', df7)
    df7['Personal Income Tax Rate'] = getCalcValue('personalIncomeTaxRate', df7)
    df7['Construction GDP'] = getCalcValue('constructionGDP', df7)
    df7['Unemployment Rate'] = getCalcValue('unemploymentRate', df7)
    df7['Minimum Wage (USD/month)'] = getCalcValue('minimumWage(USD/month)', df7)
    df7['Government Debt To GDP'] = getCalcValue('governmentDebtToGDP', df7)
    df7['Building Permits'] = getCalcValue('buildingPermits', df7)
    df7['Price To Rent Ratio'] = getCalcValue('priceToRentRatio', df7)
